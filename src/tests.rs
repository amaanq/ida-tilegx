...
#[test]
fn raw_tilegx_score_saturates_on_dense_bundles() {
...
#[test]
fn raw_tilegx_detector_rejects_arm_boot_prefix() {
   let arm_words = [
      0xEA00_0014_u32,
      0xE59F_F014,
      0xE59F_F014,
   ...
   for (chunk, word) in bytes.chunks_exact_mut(4).zip(arm_words) {
...
const REFERENCE_CORPUS_ROOT: &str = "/home/amaanq/tilegx-routerboot-from-mikrotik";

#[test]
fn raw_tilegx_detector_requires_image_relative_data_refs() {
   let bundle = [0x09, 0x18, 0x9B, 0x0D, 0xD3, 0x05, 0xCD, 0x46];
   let mut bytes = [0_u8; TG_RAW_SCORE_MAX_BYTES];
   for chunk in bytes.chunks_exact_mut(8) {
      chunk.copy_from_slice(&bundle);
   }
   // SAFETY: local byte buffer is valid and readable.
   let verdict = unsafe { tg_detect_raw_tilegx(bytes.as_ptr(), bytes.len()) };
   assert_eq!(verdict.score, 100);
   assert_eq!(verdict.runtime_base, 0);
   assert_eq!(
      verdict.accepted, 0,
      "a perfect decode score without resolvable data refs is not evidence of TILE-Gx code"
   );
}

#[test]
fn raw_tilegx_detector_rejects_degenerate_buffers() {
   for filler in [0x00_u8, 0xFF] {
      let bytes = [filler; TG_RAW_SCORE_MAX_BYTES];
      // SAFETY: local byte buffer is valid and readable.
      let verdict = unsafe { tg_detect_raw_tilegx(bytes.as_ptr(), bytes.len()) };
      assert_eq!(
         verdict.accepted, 0,
         "filler {filler:#04x} must not be accepted"
      );
   }
}

#[test]
fn raw_tilegx_detector_accepts_reference_firmware() {
   let carved = std::path::Path::new(REFERENCE_CORPUS_ROOT).join("carved");
   let Ok(entries) = std::fs::read_dir(&carved) else {
      println!(
   ...
   for entry in entries.flatten() {
      ...
      // SAFETY: heap buffer is valid and readable for its whole length.
      let verdict = unsafe { tg_detect_raw_tilegx(bytes.as_ptr(), bytes.len()) };
      assert_eq!(
         verdict.accepted,
         1,
         "{} is real TILE-Gx firmware and must be accepted (score={}, base={:#x})",
         path.display(),
         verdict.score,
         verdict.runtime_base
      );
      assert_eq!(verdict.runtime_base, TG_RAW_RUNTIME_ALIAS_BASE);

      let mut disguised = bytes.clone();
      disguised[..4].copy_from_slice(&[0x7F, b'E', b'L', b'F']);
      // SAFETY: heap buffer is valid and readable for its whole length.
      let disguised_verdict = unsafe { tg_detect_raw_tilegx(disguised.as_ptr(), disguised.len()) };
      assert_eq!(
         disguised_verdict.accepted,
         0,
         "{} must be left to the ELF loader once it carries ELF magic",
         path.display()
      );
      checked += 1;
   }
   assert!(
      checked > 0,
      "reference corpus contained no carved firmware images"
   );
}

#[test]
fn raw_tilegx_detector_rejects_elf_images() {
   for path in [
      "/home/amaanq/tilegx-mde-from-mikrotik/hv-6.48.7.clean.elf",
      "/home/amaanq/tile-vmlinux/vmlinux",
   ] {
      ...
      assert_eq!(
         verdict.accepted, 0,
         "{path} is an ELF image and belongs to the ELF loader (score={})",
         verdict.score
      );
   }
}
