...
impl<'bytes> RawTilegxHeuristic<'bytes> {
   ...
   fn evaluate(self) -> TgRawFileVerdict {
      ...
      if looks_like_known_container(self.bytes) {
         return TgRawFileVerdict::default();
      }
      ...
      // A decode score is almost no evidence on its own: the bundle encoding is
      // permissive enough that zero fill, ASCII text and AArch64 code all reach
      // 100. A non-zero runtime base is the real signal, since it means several
      // constant data references resolved back into this image.
      let runtime_base = self.runtime_base();
      TgRawFileVerdict {
         accepted: u8::from(runtime_base != 0 && score >= TG_RAW_TILEGX_ACCEPT_SCORE),
         score,
         decoded_bundles,
         total_bundles,
...
/// Leading bytes of formats that ship their own loaders: ELF, DOS/PE, and
/// Mach-O in both byte orders.
///
/// A raw-firmware heuristic must decline these outright rather than score them.
/// TILE-Gx firmware is headerless by definition, so anything carrying a
/// container magic is somebody else's file.
const CONTAINER_MAGICS: [&[u8]; 6] = [
   b"\x7FELF",
   b"MZ",
   &[0xCE, 0xFA, 0xED, 0xFE],
   &[0xCF, 0xFA, 0xED, 0xFE],
